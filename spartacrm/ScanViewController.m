...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        [HttpTask teminalsale:_string barcode_ids:@"" userid:[GlobleData sharedInstance].userid sucessBlock:^(NSString *responseStr) {
            NSError *error;
            ...
            if ([status isEqualToString:@"1"]) {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"发货成功" delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
                [alertView show];
                MainTabBarViewController *MainVc = [[MainTabBarViewController alloc] init];
                [self presentViewController:MainVc animated:YES completion:nil];
            }else{
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"出现异常" message:remark delegate:self cancelButtonTitle:@"确定" otherButtonTitles: nil];
                [alertView show];
            }
...
- (void)getString:(NSString *)codeString{
